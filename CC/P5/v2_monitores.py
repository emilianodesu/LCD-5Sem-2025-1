    ...
    def run(self):
        while True:
            with self.shared_data.condition:
                # Check if the writer has caught up to avoid producing more than consumed
                while self.shared_data.items_produced != self.shared_data.items_consumed:
                    self.shared_data.condition.wait()

                # Stop if we have produced enough items
                if self.shared_data.items_produced >= 5:
                    self.shared_data.condition.notify_all()
                    break
                ...
                # Mark the item as produced
                self.shared_data.items_produced += 1
                # Notify the writer that an item is ready
                self.shared_data.condition.notify()
        ...
        while True:
            with self.shared_data.condition:
                ...
                while self.shared_data.items_produced == self.shared_data.items_consumed:
                    ...
                    if self.shared_data.items_consumed >= 5:
                        return
                    self.shared_data.condition.wait()
                ...
                # Notify the reader that an item has been consumed
                self.shared_data.condition.notify()


if __name__ == '__main__':
    sd = SharedData('computersV2.csv')
    reader = Reader(sd)
    writer = Writer(sd)
    reader.start()
    writer.start()
